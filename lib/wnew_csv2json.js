 ...
 * @return {Array}      stream file array
 */
const csv2json = (file, opts) => {

  // merge defaults
  const ROW_SEPARATOR = (opts && opts.rowSeparator) || ROW_SEPARATOR_DEFAULT
  const COL_SEPARATOR = (opts && opts.colSeparator) || COL_SEPARATOR_DEFAULT
  const MENU_HEADER   = (opts && opts.menuHeader)   || MENU_HEADER_DEFAULT
  const EXTENSION     = (opts && opts.extension)    || EXTENSION_DEFAULT
  ...
  menuFile.path = `${menuFile.base}/${MENU_HEADER}.${EXTENSION}`
  result.push(menuFile)

  // create all main.json
  mains.map((main, index) => {
    const mainFile = file.clone()
    mainFile.contents = new Buffer(JSON.stringify(main))
    mainFile.path = `${mainFile.base}/${menus[index]}.${EXTENSION}`
...
module.exports = function(opts) {
  ...
  function transform(file, encode, callback) {
    ...
    csv2json(file, opts).map(file => this.push(file))
